...
void redBlackTreeInsertionFixup(intervalTree* node) {
	if (getParent(node) == NULL) {
		...
		//TODO: make sure the leftrotate and rightrotate procedures are called in the correct and exact moments
		/*
		if (node == g->left->right) {
			wikipediaLeftRotate(p);
			node = node->left;
		} else if (node == g->right->left) {
			wikipediaRightRotate(p);
			node = node->right;
		}
		 */
		...
		if (node == p->left) {
			p->color = 'B';
			g->color = 'R';
			wikipediaRightRotate(g);
		} else {
			p->color = 'B';
			g->color = 'R';
			wikipediaLeftRotate(g);
		}
...
//TODO: make sure this is the correct insertion for the intervals
void insertRecursive(intervalTree *root, intervalTree * n) {
...
int main() { //this implementation uses global variable tree to access its values
	...
	//TODO: leftrotate with their parent: so check it better in the rbtfixup operation
	...
	//deleteNode(searchInterval(35, 38));
	...
	if (node->parent != NULL) {
		...
		if (stepBro->color == 'R') {
			...
			if (node == node->parent->left) {
				wikipediaLeftRotate(node->parent);
			} else {
				wikipediaRightRotate(node->parent);
			}
		}
